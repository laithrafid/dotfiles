...
Copy/Paste this line in your CLI:
    ...
    curl -L  https://github.com/laith.rafid/dotfiles/raw/main/install.sh | sh
...
Set up your `local.vimrc` file with packages you want:
    ...
    echo "let g:configs_packages = ['general', 'coding', 'clojure', 'color']" > local.vimrc
...
configs settings take place. The idea was so people could add Bundle lines
without having to worry about conflicts in vimrc when pulling. [Read
More](https://github.com/zaiste/configs/pull/27).

`~/.vim/after.vimrc` will be loaded after all configuration options are set. This is the best place to change configs default behaviour (key bindings) and/or color scheme.

## Packages

Inside your local configuration file `$HOME/.vim/local.vimrc` set `g:configs_packages` variable with packages you want to use.
...
For example, if you are only interested in Ruby/Rails related stuff, put something like the following line inside `$HOME/.vim/local.vimrc`:
    ...
    let g:configs_packages = ['general', 'coding', 'ruby', 'color']
...
## Call for Help
 ...
 don’t hesitate to fork and send pull requests. Thanks!
